...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'update_result') {
        try {
            ...
            // 1.5 Update Category (Gender) for both teams
            $new_category = $_POST['category'] ?? '';
            if ($new_category && in_array($new_category, ['Men', 'Women'])) {
                // Fetch team IDs for this match
                $t_stmt = $conn->prepare("SELECT team1_id, team2_id FROM matches WHERE id = :id");
                $t_stmt->execute(['id' => $id]);
                $match_teams = $t_stmt->fetch();
                
                if ($match_teams) {
                    $upd_teams = $conn->prepare("UPDATE teams SET gender = :gender WHERE id IN (:t1, :t2)");
                    $upd_teams->execute([
                        'gender' => $new_category,
                        't1' => $match_teams['team1_id'],
                        't2' => $match_teams['team2_id']
                    ]);
                }
            }
...
$stmt = $conn->query("
    SELECT m.id, m.match_time, m.sport_id, m.status, m.winner_team, m.win_description,
           ...
           t1.name as t1_name, t1.gender as t1_gender, t2.name as t2_name, s.name as sport_name,
           ls.score_json
    FROM matches m
                    ...
                    <div class="row align-items-center gx-3">
                        ...
                        <div class="col-md-2 border-end text-center">
                            ...
                            <select name="category" class="form-select form-select-sm">
                                <option value="Men" <?= ($match['t1_gender'] ?? 'Men') === 'Men' ? 'selected' : '' ?>>Men</option>
                                <option value="Women" <?= ($match['t1_gender'] ?? 'Men') === 'Women' ? 'selected' : '' ?>>Women</option>
                            </select>
                        </div>
